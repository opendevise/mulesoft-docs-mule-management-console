...
If you are upgrading to the latest version of MMC, and your current MMC installation is configured to xref:setting-up-and-managing-users-via-ldap.adoc[manage users via LDAP], you will need to migrate your existing LDAP configuration to your new MMC installation.
...
* Learn how to xref:upgrading-to-mmc-with-an-external-database.adoc[upgrade to MMC with an external database].