...
When you run the xref:installing-the-production-version-of-mmc.adoc[production version] of MMC for the first time, it will attempt to detect any Mule runtimes on the network, but will not try to pair with any server it finds. You must manually register your Mule runtime.
