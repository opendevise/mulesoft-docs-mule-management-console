...
Custom alerts are executed as Groovy script snippets. The execution context is exactly the same as for other scripts in Mule. The context bindings for these scripts are described in xref:3.8@mule-runtime::scripting-module-reference.adoc#sts=Script%20Context%20Bindings[Script Context Bindings].
